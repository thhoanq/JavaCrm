<%@page import="com.java_crm.service.ProjectsServiceImpl"%>
<%@page import="com.java_crm.service.ProjectsService"%>
			...
			<div class="container-fluid">
				<div class="row bg-title">
					<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
						...
						<h5 class="page-title">(Gỡ task trước khi xóa project)</h5>
					</div>
					<c:if test="${role == 1 }" >
	                	<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12 text-right">
							<a href="<c:url value="/project_add" />" class="btn btn-sm btn-success">Thêm mới</a>
						</div>                		
	                </c:if>	 
					<!-- /.col-lg-12 -->
				</div>
								...
								<table class="table" id="example">
									...
									<tbody>
										<c:forEach var="proj" items="${projects }">
											<tr>
												<td>${proj.getId() }</td>
												<td>${proj.getProjectName() }</td>
												<td>${proj.getDayStart() }</td>
												<td>${proj.getDayEnd() }</td>
												<td>${proj.getProgress() }%</td>
												<td>
													<a href="<c:url value="/project_edit?id_pro=${proj.getId() }" />" class="btn btn-sm btn-primary">Sửa</a>
													<c:set var="numTask" value="${proj.getNumTask() }" />
													<c:if test="${numTask == 0 }">
														<a href="<c:url value="/api/project_delete?id_pro=${proj.getId() }" />" class="btn btn-sm btn-danger">Xóa</a>
													</c:if>
													<a href="<c:url value="/project_detail?id_pro=${proj.getId() }" />" class="btn btn-sm btn-info">Xem</a>
												</td>
											</tr>
										</c:forEach>
									</tbody>
